...
class FeatureProcessor:
    ...
    def process_video_features(self, video_data):
        ...
        quality_score = self.calculate_quality_score(
            video_data["view"], video_data["like"], video_data["favorite"]
        )
...
class VideoRecommender(keras.Model):
    ...
    def __init__(self, num_tags, num_authors, embedding_dim):
        ...
        self.deep_layer1 = keras.layers.Dense(64, activation="relu", name="deep_1")
        self.deep_layer2 = keras.layers.Dense(32, activation="relu", name="deep_2")
        self.deep_layer3 = keras.layers.Dense(16, activation="relu", name="deep_3")
    ...
    def call(self, inputs):
        ...
        deep_features = tf.concat(
            [content_features, author_embedding[:, 0, :], quality_score], axis=1
        )
...
def load_model_and_processor(save_dir):
    ...
    processor.author2idx = defaultdict(
        lambda: len(processor.author2idx), data["author2idx"]
    )
...
def train_model(model, data, labels, optimizer, class_weights):
    ...
    for features, batch_labels in dataset:
        with tf.GradientTape() as tape:
            predictions = model(
                [features["tags"], features["author"], features["quality_score"]]
            )
            ...
            weights = tf.gather(
                [class_weights[0], class_weights[1]], tf.cast(batch_labels, tf.int32)
            )
...
def evaluate_model(model, data, labels):
    ...
    """
    predictions = (
        model([data["tags"], data["author"], data["quality_score"]]).numpy().flatten()
    )
...
def main():
    ...
    processor = FeatureProcessor()
    processed_data, labels, max_tags = load_and_process_data(
        "historyVideo.json", processor
    )
...
def predict_interests(model, processor, video_data):
    ...
    for video in processed_videos:
        ...
        author_idx = (
            processor.author2idx[video["author"]]
            if video["author"] in processor.author2idx
            else 0
        )
...
def testmodel():
    ...
    for i, video in enumerate(ranked_videos, 1):
        ...
        mapped_tags = [
            f"{tag}({processor.tag2idx[tag]})" for tag in video["original_tags"]
        ]
